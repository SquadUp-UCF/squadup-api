...
```
src/
  main.ts              # bootstrap: /api prefix, CORS, static /uploads + /sports, Swagger
...
| `photo_url` | string | banner image; defaults to the sport's stock banner (`/sports/<slug>.svg`) when the host doesn't supply one |
| `createdAt` / `updatedAt` | Date | timestamps |
...
**Sport banners.** Each game has a banner image (`photo_url`). If the host
doesn't supply one, it defaults to the sport's stock banner served from
`/sports/<slug>.svg` (sport names are normalized to a slug; unknown sports get a
generic `/sports/default.svg`). Editing a game's sport re-points a stock banner
at the new sport but never overwrites a host-supplied image. The placeholder
banners under `public/sports/` are generated by `npm run banners:generate`; drop
in a real `.jpg` or `.png` with the same slug (e.g. `soccer.jpg`) and the
resolver uses it in preference to the `.svg` (a restart refreshes the listing).
`npm run banners:fetch` populates real 1200×400 photos for every sport from
Pexels (needs a free `PEXELS_API_KEY` in `.env`).
...
2. Create a `.env` from the template and fill it in:
   ...
   | `FRONTEND_URL` | Optional; base URL used to build the password-reset link. Defaults to production — set it to your local front end when testing the reset flow |
